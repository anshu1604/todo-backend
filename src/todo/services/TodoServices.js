...
class TodoServices {
    ...
    try {
      Todo.find(
        ...
        (err, data) => {
          ...
          return sendResponse(this.response, "Active Tasks List", taskList);
        }
      );
    } catch (err) {
      console.log(err);
  ...
  deleteTask() {
    try {
      ...
      Todo.findOne(
        { _id: id, status: STATUS.ACTIVE_TASK },
        async (err, data) => {
          if (err) {
            return sendErrorResponse(this.response, "", err);
          }
    ...
    try {
      Todo.find(
        ...
        (err, data) => {
          ...
          return sendResponse(this.response, "Deleted Tasks List", taskList);
        }
      );
    } catch (err) {
      console.log(err);
      ...
      Todo.findOne(
        ...
        async (err, data) => {
          ...
          if (data) {
            Todo.findByIdAndUpdate(
              id,
              { status: STATUS.DONE_TASK, last_updated: Date() },
              (err, updatedData) => {
                if (err) {
                  return sendErrorResponse(this.response, "", err);
                }
    ...
    try {
      Todo.find(
        ...
        (err, data) => {
          ...
          return sendResponse(this.response, "Done Tasks List", taskList);
        }
      );
    } catch (err) {
      console.log(err);
    ...
    try {
      ...
      Todo.findOne(
        ...
        async (err, data) => {
          ...
          return sendResponse(this.response, "Task Details", taskDetails);
        }
      );
    } catch (err) {
      console.log(err);
